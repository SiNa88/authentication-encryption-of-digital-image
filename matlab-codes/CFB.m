function [Img_Encrypted_red Img_Encrypted_green Img_Encrypted_blue Img_Decrypted]=CFB(Im,key)
tStart = tic;
            ...
            for k=1:num
                for s=1:4
                    if reply=='D' || reply=='d'        
                        ...
                        IV2=ShiftLeft2(IV2,Cell_encr{i,j}(k,s)); % Shift IV to left
                    elseif reply=='3'
                        ...
                        Cell_encr{i,j}(k,s)=bitxor(IV_Encrypt(1,1),Cell{i,j}(k,s));
                        IV2=ShiftLeft(IV2,Cell_encr{i,j}(k,s)); % Shift IV to left
                    end    
                end
            end
        end
            ...
            for k=1:num
                for s=1:4
                    if reply=='D' || reply=='d'        
                        ...
                        IV2=ShiftLeft2(IV2,Cell_Encryption{i,j}(k,s)); % Shift IV to left
                    elseif reply=='3'
                        IV_Encrypt=fun1(IV2,RoundKey1,RoundKey2);
                        CellDecryptedPlain{i,j}(k,s)=bitxor(IV_Encrypt(1,1),Cell_Encryption{i,j}(k,s));
                        IV2=ShiftLeft2(IV2,Cell_Encryption{i,j}(k,s)); % Shift IV to left
                        ...
                        IV_Encrypt=fun1(IV2,RoundKey,S_Box);
                        CellDecryptedPlain{i,j}(k,s)=bitxor(IV_Encrypt(1,1),Cell_Encryption{i,j}(k,s));
                        IV2=ShiftLeft(IV2,Cell_Encryption{i,j}(k,s)); % Shift IV to left
                    end
                end
            end
        end
...
%         subplot(1,3,3), imshow(uint8(Img_Decrypted)),title('Decrypted Image');
end
tEnd = toc(tStart);
